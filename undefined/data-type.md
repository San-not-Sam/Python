...
```python
s = "Life is too short."
s.split("too") # 괄호안에 아무 것도 쓰지 않으면 공백을 기준으로 자른다. 
```
...
* \[특징]&#x20;
  1. \[양수] 원소의 순서는 맨 왼쪽부터 '0'에서 시작한다. e.g. (0, 1, 2, 3, ...)
  2. \[음수] 원소의 순서는 맨 오른쪽부터 '-1'에서 시작한다. e.g. (..., -3, -2, -1)
...
```python
# 빈 리스트 생성 후 3, 2, 1 순서대로 원소를 추가하기
L = []
L.append(3)
L.append(2)
L.append(1)
...
* list와의차이점은 다음 두 가지이다.&#x20;
  1. 원소를 감쌀때, 리스트는 \[]을 튜플은 ()을 사용한다.&#x20;
  2. 리스트는 생성 후 변경이 가능한 반면, 튜플은 생성 후의 변경이 불가하다.
     * mutable : 생성 이후에 변경(assignment)이 가능한 데이터 타입 e.g. **list**, **dict**, **set**
     * immutable : 생성 이후에 변경이 불가한 데이터 타입  e.g. int, float, **string**, tuple, frozenset
...
* 집합의 연산자인 '교집합', '합집합', '차집합'이 지원된다.
* \[특징] 집합의 특징이 2가지 있는데, 이 특징이 리스트와의 차이점이라 사용한다. 첫번째 특징이 집합 자료형을 사용하는 주된 이유이다.
  1. 원소의 중복을 허용하지 않는다.
  2. 집합은 원소의 순서가 존재하지 않는다.&#x20;
...
```python
s.remove(13) # 집합 내에 존재하지 않는 원소를 제거하면 Error
s
```
...
* 모든 연속형 데이터 타입에서 특정 데이터의 존재 유무를 확인케 한다.&#x20;
